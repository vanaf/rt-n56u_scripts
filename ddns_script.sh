...
logger -t ddns-script "IPV4_TUN0: $IPV4_TUN0"
logger -t ddns-script "IPV6_BR0: $IPV6_BR0"
...
logger -t ddns-script "IPV4: $IPV4"
...
if [[ -z "$EXISTING_OUR_RECORD_A" ]]; then
	...
	logger -t ddns-script "A record added. Results: $WGET_RES"
else
	EXISTING_OUR_IPV4=$(extract_value "$EXISTING_OUR_RECORD_A" "content")
	if [[ "$EXISTING_OUR_IPV4" != "$IPV4" ]]; then
		logger -t ddns-script "$EXISTING_OUR_IPV4 != $IPV4"
		...
		logger -t ddns-script "A record modified. Results: $WGET_RES"
	fi
fi
...
if [[ -z "$EXISTING_OUR_RECORD_AAAA" ]]; then
	...
	logger -t ddns-script "AAAA record added. Results: $WGET_RES"
else
        EXISTING_OUR_IPV6=$(extract_value "$EXISTING_OUR_RECORD_AAAA" "content")

        if [[ "$EXISTING_OUR_IPV6" != "$IPV6_BR0" ]]; then
                logger -t ddns-script "$EXISTING_OUR_IPV6 != $IPV6_BR0"
                ...
                logger -t ddns-script "AAAA record modified. Results: $WGET_RES"
